# @summary Sets up a K3 instance - either as a node or as a server (copied from voxpupli k8 example)
#
...
class k3s (
  ...
  K3s::Proxy_method $manage_kube_proxy = true,
  ...
  K3s::Node_auth $node_auth = 'bootstrap',
  ...
  K3s::CIDR $service_cluster_cidr                    = '10.1.0.0/24',
  K3s::CIDR $cluster_cidr                            = '10.0.0.0/16',
  Stdlib::IP::Address::Nosubnet $api_service_address = k8s::ip_in_cidr($service_cluster_cidr, 'first'),
  K3s::IP_addresses $dns_service_address             = k8s::ip_in_cidr($service_cluster_cidr, 'second'),
  Stdlib::Fqdn $cluster_domain                       = 'cluster.local',
  String[1] $etcd_cluster_name                       = 'default',

  Optional[K3s::Node_role] $role = undef,
  Optional[K3s::Firewall] $firewall_type = undef,
